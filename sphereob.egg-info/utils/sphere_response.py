...
class sphereresponse(object):
    def __init__(self):
        ...
        #self.delta_x = math.floor(abs((self.prof_end - self.prof_start))/ (self.interval-1)) # Number of times field is calculated along profile
        #self.delta_x = math.floor((self.profile_length / (self.interval - 1)))
    ...
    def calculate(self):
        ...
        def dh_tot_step(mtx, dipole_m, rtx, rsp, mu, sigma_ob, thick_ob, t, o, sigma_sp, a, T):
            ...
            testingthisthing = np.array([val[0] + (ob_array[0] * thetaz), val[1] + (ob_array[1] * thetaz), val[2] + (ob_array[2]*thetaz)])
        ...
        def h_total_step_1storder(
            ...
            if hasattr(self.wave,"__len__"):
                ...
                for i in list(range(0, N - 1, 10)):
                    ...
                    tester = 2 * math.pi * (a ** 3) * (self.P/45)

                    tester2 = (dh_tot_step(self.mtx, self.dipole_m,[0, 0, self.rtx[2]], [-self.rtx[0], -self.rtx[1], self.rsp[2]],self.mu, self.sigma_ob, self.thick_ob, t,
                                          (-self.P * (i) / (N - 2)), self.sigma_sp, self.a, self.T) + H2x)
                    tester3 = (self.wave[i+1] - (self.wave[i])/(8.14*1e-6))
                ...
                # statics = static(msp, (np.array([rtx[0], rtx[1], rtx[2]]) -
                #                            (np.array([rsp[0], rsp[1], rsp[2]]))))
        ...
        self.delta_x = math.floor(abs((self.prof_end - self.prof_start)) / (self.interval - 1))
