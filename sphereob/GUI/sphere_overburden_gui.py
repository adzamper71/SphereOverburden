...
class OptionsMenu(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.user_profile.setText('-1000,1000')
        ...
        coeff_grid.addWidget(QtWidgets.QLabel('Sphere Position (m)'), 0, 2) #a_sp
        ...
        coeff_grid.addWidget(QtWidgets.QLabel('Profile start stop'), 6, 2)
    ...
    def read_tem(self):
        fileName, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Select CSV File:', "", "CSV data files (*.TEM)")
        if fileName:
            self.xs = 1
            data = pd.read_csv(fileName, sep='\s+', header=None, skiprows=10, error_bad_lines=False)
            data2 = data.dropna()
            self.TEM = data2.rename(columns=data2.iloc[0]).drop(data2.index[0])

            del self.TEM['CALCTHIS']
            self.lineID = self.TEM.NORTH.unique()
            self.lineBox.addItems(self.lineID)
...
class AppForm(QtWidgets.QMainWindow):
    ...
    def calculate_data(self):
        ...
        sphere.prof_start = [int(n) for n in self.options_menu.user_profile.text().split(',')][0]
